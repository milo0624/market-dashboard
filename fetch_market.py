...
def fetch():
    ...
    secret_key = os.environ["FUBON_SECRET_KEY"]

    sdk = FubonSDK()
    res = sdk.login_with_api_key(api_key, secret_key)
    ...
    # 指數
    indices = {}
    for sym, key, name in [("2330","TSM","台積電"),("t00","TWII","台股 TAIEX")]:
        try:
            d = rest.intraday.quote({"symbol":sym}) if key=="TSM" else rest.intraday.quote({"symbol":sym,"type":"INDEX"})
            indices[key] = {
